...
class FormFieldsTagLib {
    ...
    /*
     * for a given property in a domain class generates a link ref to the
     * referenced property's  controller with show action
     *
     */
    ...
    def allbsf = { attrs ->
        ...
        try {
            ...
            if (domainClass) {
                ...
                out << render(template: "/templates/_fields/editproplist", model: [domainClass: domainClass, domainProperties: properties]) { prop ->
                    BeanPropertyAccessor propertyAccessor = resolveProperty(bean, prop.name)
                    Map model = buildModel(propertyAccessor, attrs, 'HTML')
                    out << fieldbsf([bean: bean, property: prop.name, prefix: prefix])
                }
    ...
    private Map buildColumnModel(BeanPropertyAccessor propertyAccessor, Map attrs) {
        ...
        return [
                ...
                name              : propertyAccessor.pathFromRoot, // To please legacy _(orig)table.gsp
                property          : propertyAccessor.pathFromRoot,
                type              : propertyAccessor.propertyType,
                defaultLabel      : labelText, // To please legacy _(orig)table.gsp
                label             : labelText,
